...
public class Board {
    ...
    public void makeMove(Move move) {
        ...
        // Go through the bottom row up in the column
        for(int row = Board.NUM_ROWS-1; row >= 0; row--) 
        {
            Player p = getTile(row, column);
            if(p == null)
            {
                /**
                 * This would be the first row that is available to place
                 * a disk. Add player to the board!
                 */
    ...
    public Move[] getPossibleMoves(Player p) {
        ...
        {
            for(int column = 0; column <= Board.NUM_COLS; column++)
            {
                /*
                Go through all the possible positions to check if occupied.
                */
                // One possible way...
                //if((p.compareTo(getTile(row, column))==0) 
                //        && ((p.opponent().compareTo(getTile(row, column))==0)))
                if(getTile(row, column)==null)
                {
                    // possibleMove.add(new Move(p, row))
                }
